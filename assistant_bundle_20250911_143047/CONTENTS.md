...
**size:** 5078 bytes &nbsp;&nbsp; **sha1:** `cf8090201722be6d7a76c38cac4191c7c881f758`
...
if command -v zip >/dev/null 2>&1; then
  ...
  # Используем PowerShell Compress-Archive как запасной вариант.
  WINPWD=$(pwd -W 2>/dev/null || pwd)
  powershell.exe -NoProfile -Command "
    Set-StrictMode -Version Latest;
    \$ErrorActionPreference = 'Stop';
    Set-Location -LiteralPath '$WINPWD';
    \$list = (Get-Content -Raw '$OUT/_meta/include_files.list' -EA SilentlyContinue);
    \$paths = \$list -split \"`n\" | Where-Object { \$_ -and (Test-Path \$_) };
    if (-not \$paths) { \$paths = @(); }
